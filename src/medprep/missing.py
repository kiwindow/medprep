...
1. **「測っていない」こと自体が情報である。**
   ...
   ただし多くの欠測は単なる入力漏れである。そこで欠損指示子（`add_indicator`）は
   **既定では特徴量にせず**、欠測の位置は書き出しの「欠損値の位置」シートに残す。
...
def analyze(
    ...
    *,
    ...
    rep.notes.append(
        "欠損指示子（add_indicator）は**既定では特徴量にしない**。"
        "『測っていない』こと自体が情報であることはあるが、多くは単なる入力漏れで、"
        "欠測率の低い列の指示子はほぼ定数になり正則化モデルを不安定にするだけである。"
        "欠測がどこにあったかは 5_training_data・6_test_data（本コード専用） の「欠損値の位置」シートに 0/1 で"
        "残るので情報は失われない。特徴量として使いたいときは "
        "policy={'missing': {'add_indicator': True}} と明示すること")
