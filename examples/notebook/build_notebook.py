...
#   元のデータと症例ごとに結合し直せなくなる。
#   True にすると、**全てのセルが空欄の行だけ**を削除する。
#   そのときは「第何行目を消したか」が 除外の記録.xlsx の「削除した行」に残る。
...
rep = mp.autoprep(
    ...
    drop_empty_rows=DROP_EMPTY_ROWS,     # 既定 False（★1 行も削除しない★）
...
| 種類 | 何が減るか |
|---|---|
| **列** | ID・重複列・自由記載は解析から外す。日付は特徴量にしない（→ 3) 列の処理） |
| **行** | ★1 行も削除しない。★ 外すのが望ましい症例に **`除外推奨` の印**を付けるだけ |
| **値** | 欠損コード（999）・あり得ない値を NaN にする（★行は消えない★） |

> **ひとつだけ例外がある ―― `DROP_EMPTY_ROWS = True`**
>
> **全てのセルが空欄の行**は症例ですらない。消したい人のために選べるようにしてある。
> 既定は `False`。`True` にしたときは、
> `table/除外の記録.xlsx` の **「削除した行」シート**に
> **第何行目を消したか**が残る（`元の行` と `Excel の行`）。
>
> `False` のままでも、**空欄だけの行があれば段の表で必ず知らせる。**
> 黙って残すのも、黙って消すのも、どちらも同じくらい困るからである。
...
code("""
# ★削除した行★（DROP_EMPTY_ROWS = True のときだけ中身が出る）
rep.dropped_rows if rep.dropped_rows is not None else '削除していない（1 行も消していない）'
""")

code("""
print(f'元のデータ {len(df)} 行 → 掃除済み {len(rep.df_clean)} 行')
print(f'外すのが望ましい行: {rep.n_excluded} 例（印だけ）')
print()
rep.df_clean.loc[rep.df_clean['除外推奨'] == 1,
                 ['仮名ID', '除外推奨', '除外推奨_理由']].head(100)
""")

md("""
### 行の処理の一覧

`table/除外の記録.xlsx` の **「外すのが望ましい症例」** に、
同じ症例が**全件**入っている。理由のほかに**日付の列も添えてある** ――
除外の理由はほとんど日付の矛盾なので、**理由だけ見せられても確かめようがない**からである。

| シート | 中身 |
|---|---|
| まとめ | 何を、いくつ、なぜ減らしたか（行・列・値の集計） |
| （削除した行） | ★`DROP_EMPTY_ROWS = True` のときだけ作られる★。`元の行` と `Excel の行` |
| **外すのが望ましい症例** | **1 症例 1 行**。ID・理由・関係する日付 |
| **解析から外す列** | **本当に消える列だけ**。列名・理由・どの段で消えたか |
...
code("""
# ファイルに出した一覧（Excel と同じもの）
mp.excluded_cases(rep)
""")

md("""
## 3) 列の処理

列の扱いには**二通り**ある。**消える列と、残るが使わない列は別のものである。**
...
日付や生存時間の列は後者である。**そのままでは説明変数にならない**が、
症例を確かめるのに要るので**ファイルには残す。**

この二つを一緒に並べると、「消えた」と言われた列が手元のファイルにあって
**人が混乱する。** だから 3 枚目のシートには**前者だけ**を入れてある。

★`掃除済みデータ.xlsx` には全列がそのまま残る。★ あれは
**人が元の記録と突き合わせる**ためのものなので、ID も自由記載も抜かない。
消えるのは `解析用データ.xlsx` から先である。
""")

code("""
# 本当に消える列だけ（Excel の 3 枚目と同じもの）
mp.removed_columns(rep)
""")
...
同じデータを **4 段階**で書き出してある。**施した処置が 1 段ずつ違う。**
どれを使うかで結果が変わるので、ここを取り違えないこと。

| # | ファイル | 行 | 列 | 何のため |
|---|---|---|---|---|
| ① | `data/元データ.xlsx` | **元と同じ** | **全列** | **手を触れていない元のまま。** 掃除の前後を突き合わせる |
| ② | `data/掃除済みデータ.xlsx` | 元と同じ<sup>※</sup> | **全列＋派生列** | **人が読む。** 元の記録と症例ごとに突き合わせる（ID が残っている） |
| ③ | `data/解析用データ.xlsx` | 元と同じ<sup>※</sup> | 落とす列を除く | **自分で解析する。** 行が減っていないので元データと `axis=1` で結合できる |
| ④ | `data/前処理済み_train.xlsx` / `_test.xlsx` | ★部分集合★ | 特徴量のみ | **モデルに渡す。** 元の行番号と ID が付いている |

<sup>※</sup> `DROP_EMPTY_ROWS = True` にしたときだけ、**全セルが空欄の行の分だけ減る。**
そのときは「書き出したデータの説明」の `行` の欄に何行減ったかが出る。

### ①〜④ は、どれも **1 枚のシートではない**

| ファイル | シート |
|---|---|
| ① 元データ | 1 枚だけ（**手を触れていない証拠**） |
| ② 掃除済みデータ | **データ** ／ **列の扱い**（列ごとの役割と判断の根拠） ／ **減らしたもの** |
| ③ 解析用データ | **データ** ／ **0と1の対応**（どの水準を 1 にしたか） |
| ④ 前処理済み_train / _test | **標準化後** ／ **生の値** ／ **欠損値の位置** |

④ の 3 枚は、**同じ症例・同じ列**を 3 通りの見方で並べたものである。

| シート | 中身 |
|---|---|
| **標準化後** | モデルに渡す行列。施設は `施設=B院` のダミー、数値は z 値 |
| **生の値** | ★標準化する前の値★。施設は `A院` のまま |
| **欠損値の位置** | ★補完する前に、どこが空欄だったか★を 0/1 で。最後の列は 1 症例あたりの欠損数 |

標準化後の表は計算のためのもので、人が読んでも意味が取れない。
**群間比較をしたり、値を目で確かめたりするには「生の値」のシートを使う。**

### 欠損指示子を特徴量にしない理由

「測っていない」こと自体が情報であることはある（安定していたから測らなかった、
重症で測れなかった）。しかし**多くの欠測は単なる入力漏れ**で、意味を持たない。
欠測率 0.5% の列の指示子はほぼ定数になり、**正則化モデルを不安定にするだけ**である。

そこで `medprep` は、欠損指示子を**既定では特徴量にしない**。
代わりに**欠測の位置を 0/1 でそのまま残す**（「欠損値の位置」シート）ので、
情報は 1 つも失われない。意味のある欠測だと判断したら、その列だけを
自分で結合して特徴量にすればよい。

全列について指示子を作らせたいときは、`autoprep(..., policy={'missing':
{'add_indicator': True}})` と**明示する**。

### 段ごとに、何が足されるか

```
① 元データ    何もしていない。読み込んだそのまま
   ↓
② 掃除済み    ⓪ 全セルが空欄の行を削除（★DROP_EMPTY_ROWS=True のときだけ★）
              ⓪b 生存時間の 3 列を指定していれば duration / event / _start / _end を追加
              ① 欠損コード（999 等）と「未測定」を NaN に
              ② 検出限界（<0.1）を数値化      ③ 単位の混在を換算
              ④ 生理学的にあり得ない値を NaN に ⑤ 派生指標を追加（補正Ca・透析年数 等）
              ⑥ 日付を解釈（和暦・全角・シリアル値）
              ⑦ 外すのが望ましい行に印（★行は削除しない★）
   ↓
...
**⑨ でだけ行が減る**（⓪ を選ばなければ）。
だから前処理済みにだけ `元の行` と ID の列を付けてある。

### データ以外に保存されるもの

| 置き場所 | 中身 |
|---|---|
| `table/除外の記録.xlsx` | 何を減らしたか（まとめ／削除した行／外すのが望ましい症例／解析から外す列） |
| `table/書き出したデータの説明.xlsx` | 下のセルで出る表そのもの |
| `table/prep_tables.xlsx` | 役割・欠損・外れ値・Table 1・群間比較・管理目標を**1 冊に**まとめたもの |
| `table/Table1_2.xlsx` / `.docx` | 論文用の Table 1・Table 2（日本語／English） |
| `table/table1_詳細.xlsx` | 検定の選択理由・効果量・多重比較の補正 |
| `model/schema.yaml` | ★再現性の中核★。列の役割と判断の根拠 |
| `model/medprep_version.txt` | どの版で作ったか |
| `figure/*.png` | 欠損の地図・分布・相関・主成分・管理目標 |
| `report/prep_report.html` | 上の全部を **HTML 1 枚**に |
...
**算出した `duration`（観察期間）・`event`（1=発生 / 0=打ち切り）・
`_start`・`_end` は、掃除済みデータと解析用データにも列として残る。**
`rep.survival.data` の中にしか無いと、Excel を開いた人には見えないからである。
生存時間に変換できなかった症例は **NaN**（★行は削除しない★）。
なお `duration` と `event` は**説明変数には入らない**（目的変数そのものだから）。
...
d = sf.data.rename(columns={
    ...
    'CRP定量': 'CRP', '無機リン(P)': 'P', '透析年数': 'vintage'})
...
>
> `medprep` は手元だけで完結し、外に何も送らない。
> 結果の置き場所の根には毎回 `.gitignore` を入れているが、
> **人に渡すときは中身を自分の目で確かめること。**

1. 「4) データの入力」のボタンで、自分のファイル（.xlsx / .xls / .csv）を選ぶ
2. 環境変数セルの `ID_COL` / `GROUP` / `DATE_COL` / `SURVIVAL_DATES` / `OUTCOME` を
   自分の列名に書き換える（**空にしておけば、読み込んだあとに一覧から選べる**）
3. このノートブックを上から実行する
