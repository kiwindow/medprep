...
mp.build_report(df, sch, audit=aud, missing=ms, outliers=ol, table1=t1,
                figures=figs, path="prep_report.html")
```

図も表も 1 ファイルに埋め込むので、そのまま送れてそのまま開けます。
**警告は先頭にあります**（致命的な所見が 10 枚目にあるレポートは読まれません）。
...
**症例レベルの値は既定では出しません。** レポートはメールで回るからです
（`show_values=True` を明示したときだけ値が載り、その旨がレポートに表示されます）。

図は 1 つの検証済み色体系で描いています。**相関は発散配色（中点の灰色が「関連なし」）、
